...
Exit codes:
0 = success (at least one resource inspected and all valid)
1 = error (zero resources inspected or any invalid entry)
"""
...
def find_violations(known: set[str]) -> list[str]:
    ...
    if inspected_count == 0:
        findings.append("zero_resources insulin target pattern matched nothing")
    return findings
...
def main() -> int:
    ...
    if findings:
        for finding in findings:
            print(finding)
        print(f"\n{len(findings)} error(s) found (0 resources inspected)}")
        return 1
