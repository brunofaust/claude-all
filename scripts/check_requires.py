...
def load_resource_keys() -> set[str]:
    ...
    def find_violations(known: set[str]) -> list[str]:
        """Return one finding per dangling/malformed ``requires`` entry plus a success summary."""
        findings: list[str] = []
        inspected = 0
    ...
    if not findings and inspected > 0:
        print(f" ✔ {inspected} claude-all.json inspected, all requires valid.", file=sys.stderr)
    return findings
