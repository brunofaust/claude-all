...
def main() -> int:
    ...
    # Discover manifests to check for zero-discovery failure
    manifests = sorted((SRC / "claude_all").rglob("claude-all.json")) + sorted(
        (SRC / "claude_all").rglob("*.claude-all.json")
    )
    if not manifests:
        print(
            "0 resources matched the discovery pattern (claude-all.json under src/claude_all) — "
            "a dependency manifest that references no resources is unsafe.",
            file=sys.stderr,
        )
        return 1
