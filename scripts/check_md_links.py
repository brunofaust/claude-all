...
class MarkdownLinkChecker:
    ...
    def output_results(self, json_mode=False):
        if json_mode:
            result = {
                'status': 'pass' if not (self.broken_links or self.unlinked_resources) else 'fail',
                'counts': {
                    ...
                    'vendored_files_skipped': self.counts['vendored_files_skipped']
                },
                'broken_links': [
                    {
                        'file': link['file'],
            ...
            for link in self.broken_links:
                print('{file}:{line}: broken-link -> {link}'.format(**link))
