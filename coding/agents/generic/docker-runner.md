...
description: >-
  ...
  `docker-compose.yml` / `compose.yaml`), captures stdout+stderr, and returns a TIGHT summary —
  image tag + size + duration for builds; container ID + name + status + ports for runs/ps; tail of
  relevant log lines (default last 50) for logs; exit code + last error for failures. NEVER runs
  destructive operations without explicit confirmation in the user's prompt — that means `docker
  rm`, `docker rmi`, `docker volume rm`, `docker network rm`, `docker system prune`, `docker compose
  down -v` (the `-v` removes volumes), `docker push`. Read + run only by default. Do NOT use for:
  editing Dockerfile / compose.yaml (Sonnet), choosing base images / writing new Dockerfile content
  (Sonnet), or Kubernetes (`kubectl` is different — use main session or a future k8s-runner agent).
