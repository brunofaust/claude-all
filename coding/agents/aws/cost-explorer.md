...
description: >-
  ...
  forecast, anomalies, and savings plan utilization. Triggers on "AWS spend", "show costs", "how
  much did <service> cost", "cost by tag", "where is the money going", "forecast next month", "find
  cost anomalies", "AWS bill breakdown". Read-only. Use for billing investigation, cost attribution,
  and budget tracking. Note: Cost Explorer API has per-request charges ($0.01 each) so use it
  judiciously. Do NOT use for: invoice details (use Billing console), budgets/alerts setup (main
  session), or commitment purchases.
