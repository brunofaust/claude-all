...
| Deciding whether to run on the **free-threaded (GIL-disabled) build (PEP 703)** — when to adopt it vs `run_in_thread`/subinterpreters, pros/cons, how to run + verify at runtime, `cp314t` dependency compatibility audit, and the data-integrity/Security implications of losing the GIL | [`references/free-threading.md`](references/free-threading.md)                                          |
