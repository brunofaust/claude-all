...
| Skill                   | Description                                                                                                                                                                                                                                                                                                                      |
| ----------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| brunofaust-python-style | Modern Python 3.14+ coding standards for async-first, type-safe production code. Pydantic validation at every boundary (Lambda events, ECS env vars), flaky-test prevention via test data isolation (dynamic DB ids, per-test ownership, no cross-tenant FK, mandatory `pytest-xdist`), and scoped global processes (run-for-one parameter + scope-aware DynamoDB idempotency where a global run supersedes a customer run). Reference files per topic (type-hints, error-handling, async-patterns, config, testing, data-modeling, scoped-processes, …).                                                                                                                                                                                                                                                 |
