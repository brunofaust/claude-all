...
All four kept under one `frontend/` folder — React-specific items are a subset of frontend work. Filter with `claude-all skills react` if you only want the React-named ones. These three skills and `web-design-guidelines` are **vendored from Vercel** ([agent-skills](https://github.com/vercel-labs/agent-skills), [web-interface-guidelines](https://github.com/vercel-labs/web-interface-guidelines)) — see `vendored.json` + each skill's `ATTRIBUTION.md`, and update via `scripts/vendor_sync.py`.
