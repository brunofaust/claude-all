...
| `supply-chain-guard.py`        | `PreToolUse`  | Non-blocking reminder on package-install commands (`npm`/`pnpm`/`yarn`/`bun install\|add\|ci`, `pip`/`pipx install`, `uv add`/`uv pip install`, `poetry add`). Flags supply-chain risks and the safer invocation: git/URL sources (bypass registry review), missing `--ignore-scripts` (lifecycle hooks run arbitrary code), a bare `install` with a lockfile present (use `npm ci` / `--frozen-lockfile`), and a new-package cooldown reminder. Reinforces the `research-before-build` + `security-audit` supply-chain criteria. Bypass: `CC_SUPPLY_CHAIN_OK=1`. |
