...
- **Generic boundaries, per-stack tooling.** `brunofaust-python-style` + `prek` are the Python
  *reference* (ruff / mypy / import-linter / banned-api / interrogate / vulture / bandit / gitleaks);
  ...
  Frontend lens → `brunofaust-frontend-style` (which routes to the vendored
  `composition-patterns` / `web-design-guidelines` / perf skills) + `seo`.
...
**`brunofaust-frontend-style` is the entry point** — one skill for React/browser work, the counterpart to `brunofaust-python-style`. It folds the formerly-separate `react-correctness`, `react-testing` and `web-security` skills in as its `references/`, and *routes* to the four **vendored** skills below rather than copying them, so `scripts/vendor_sync.py` keeps pulling upstream improvements. Installing it pulls the four in automatically (declared in its `claude-all.json`).

The vendored four come from Vercel ([agent-skills](https://github.com/vercel-labs/agent-skills), [web-interface-guidelines](https://github.com/vercel-labs/web-interface-guidelines)) — see `vendored.json` + each skill's `ATTRIBUTION.md`, and update via `scripts/vendor_sync.py`. The registry also carries `watch` entries for skills *derived* from upstream repos (synthesized, not copied — e.g. `adversarial-verification`): the sync never touches their files, it just reports upstream commits since the last review (`--ack <id>` after reviewing).

| Skill                         | Description                                                                                                                                                                                                                                                                                     |
| ----------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **brunofaust-frontend-style** | **The frontend entry point.** One skill routing to every dimension: `references/react-correctness.md` (useEffect when-NOT-to-use, state-location tree, derived state, stale closures, keys, React 19 hooks), `references/react-testing.md` (RTL query priority, `userEvent`, MSW, anti-snapshot, axe), `references/web-security.md` (XSS, `safeUrl()`, env-var leak table, httpOnly sessions, CSP+nonces), and `references/audit.md` — the judgment checklist `/ship` + `/ship-pr` run on every changed frontend file. Ships one consolidated `PreToolUse`/`Edit\|Write` reminder hook (once per session, source **and** test files) and one `claude_md.md`, replacing the three it absorbed. `requires` the four vendored skills below. |
| react-best-practices          | *(vendored)* React + Next.js performance patterns from Vercel Engineering — bundle optimization, data fetching, render performance.                                                                                                                                                             |
| composition-patterns          | *(vendored)* React composition patterns that scale — replace boolean prop sprawl, build flexible component libraries.                                                                                                                                                                           |
| react-view-transitions        | *(vendored)* React View Transition API — `<ViewTransition>`, `addTransitionType`, route/list/shared-element animations.                                                                                                                                                                         |
| web-design-guidelines         | *(vendored)* UI/UX/accessibility review checklist — design audits, "review my UI", "check accessibility".                                                                                                                                                                                       |
...
| `worktree-isolation-guard.py`  | `PreToolUse`  | Pauses (`permissionDecision: "ask"`) an `Edit`/`Write`/`MultiEdit` **only** when the target is in the PRIMARY checkout of a git repo (not a linked `git worktree`) AND HEAD is on a protected branch (`main`/`master`) — the exact case where parallel sessions on one checkout corrupt each other. Feature branches, linked worktrees, and non-repo files pass silently. Env: `CLAUDE_ALL_ALLOW_MAIN_EDITS=1` (opt out), `CLAUDE_ALL_PROTECTED_BRANCHES` (override the branch set). |
| `mock-spec-guard.py`           | `PreToolUse`  | Non-blocking reminder (matcher `Edit\|Write\|MultiEdit`) when a **Python test file** gains a **bare** `MagicMock()` / `AsyncMock()` — one created without `spec=` / `spec_set=` / `autospec=` / `wraps=`. Mock drift is the #1 silent-failure class: a bare mock accepts any signature, so a change to the real function's shape won't fail the test. Steers toward `autospec=True` / `spec=RealClass` / `create_autospec` at generation time. Silent unless the pattern is present, and only on test files. Env: `CLAUDE_ALL_MOCK_SPEC_OK=1` (opt out). |
| `test-data-isolation-guard.py` | `PreToolUse`  | Non-blocking reminder (matcher `Edit\|Write\|MultiEdit`) when a **test file** hard-codes a **tenant/scope id literal** — `org_id`/`tenant_id`/`account_id`/`project_key`/… `= <int or quoted string>`. Shared or hard-coded ids make tests fight over the same rows (flaky under `xdist`) and let a foreign key cross a tenant boundary — a bug class that reaches production. Steers toward dynamic ids from a factory/DB, per-test ownership, FKs within one tenant. A value that references a fixture/variable/call (`org_id=org.id`) is not a literal and never matches. Silent otherwise; deduped per (session, file). |
| `python-orjson-guard.py`       | `PreToolUse`  | **Hard-blocks** (exit 2, matcher `Edit\|Write\|MultiEdit`) a production `.py` edit that introduces stdlib `json` — `import json` / `from json import` / `json.loads\|dumps\|load\|dump(`. The edit-time layer of the `brunofaust-python-style` skill's orjson rule (complements its prek/ruff `banned-api` CI layer), steering Claude to `orjson.loads` / `orjson.dumps`. Exempt: non-`.py`, and `tests`/`scripts`/`migrations`/`alembic` dirs or `test_*` basenames. Override: `# guard:allow` (one serde/codec boundary) or `CLAUDE_ALL_ALLOW_STDLIB_JSON=1`. |
| `python-structlog-guard.py`    | `PreToolUse`  | **Hard-blocks** (exit 2, matcher `Edit\|Write\|MultiEdit`) a production `.py` edit that introduces stdlib `logging` — `import logging` / `from logging import` / `logging.getLogger`. The edit-time layer of the `brunofaust-python-style` skill's structlog rule, steering Claude to `structlog.get_logger()`. Exempt: non-`.py`, and `tests`/`scripts`/`migrations`/`alembic` dirs or `test_*` basenames. Override: `# guard:allow` (the logging-bootstrap module) or `CLAUDE_ALL_ALLOW_STDLIB_LOGGING=1`. |
| `python-settings-env-guard.py` | `PreToolUse`  | **Hard-blocks** (exit 2, matcher `Edit\|Write\|MultiEdit`) a production `.py` edit that reads config via `os.environ.get(` / `os.getenv(`. The edit-time layer of the `brunofaust-python-style` skill's config rule, steering Claude to the typed `Settings` singleton (validated once at startup). Exempt: non-`.py`, and `tests`/`scripts`/`migrations`/`alembic` dirs or `test_*` basenames. Override: `# guard:allow` (`settings.py` itself) or `CLAUDE_ALL_ALLOW_OS_ENV=1`. |
| `python-thread-subprocess-guard.py` | `PreToolUse` | **Hard-blocks** (exit 2, matcher `Edit\|Write\|MultiEdit`) a production `.py` edit reaching for a raw concurrency primitive: `asyncio.create_subprocess` (→ `run_exec()`/`run_shell()` from the subprocess owner), `asyncio.to_thread` (→ `run_in_thread()` from the thread-pool owner), or `ThreadPoolExecutor` (→ the `ThreadPool` owner wrapper). The edit-time layer of the `brunofaust-python-style` skill's external-system-ownership rule; the block message names which construct was hit and its replacement. Exempt: non-`.py`, and `tests`/`scripts`/`migrations`/`alembic` dirs or `test_*` basenames. Override: `# guard:allow` (the owner module) or `CLAUDE_ALL_ALLOW_RAW_CONCURRENCY=1`. |
...
| `secret-leak-guard.py`         | `PreToolUse`  | **Hard-blocks** (exit 2) a `git add`/`commit`/`push` that would leak credentials — a staged credential FILE (private key, real `.env`, `.aws/credentials`, `.netrc`; `.env.example` / `.pub` allowed), or a live sensitive env-var VALUE (name matching `SECRET`/`TOKEN`/`PASSWORD`/`*_KEY`/`CREDENTIAL`, length ≥ 12) appearing verbatim in the outgoing diff — the value-in-content gap `gitleaks`'s pattern scan misses. Reports only the file/variable NAME, never the value. Fails open if the repo can't be resolved; `GUARD_OK=1` / `# guard:allow` override. |
...
| `python-style-skill-loader.py` | `SessionStart` | Non-blocking reminder (matcher `""`) to invoke the `brunofaust-python-style` skill when the session's cwd looks like a Python project (`pyproject.toml`/`setup.py`/`setup.cfg`/a top-level or `src/` `*.py`). Silent in non-Python sessions. The proactive bookend to the skill's own edit-time reminder hook — it dedups on its **own** flag, independent of the edit-time hook, so the high-signal first-`.py`-edit reminder still fires even after this session-start nudge (at most one of each per session; they never pile onto the same edit). Install separately from the skill (`claude-all --user hooks python-style-skill-loader`). |
...
| `response-style`  | Concise, checkpointed replies — one focused change per turn, summarize instead of dumping file bodies, so big audits/refactors don't blow the output-token limit. |
| `worktree-isolation` | Branch/worktree before editing a shared repo; never tree-wide git writes (`stash` / `reset --hard` / `clean`) that wipe another session's work. Pairs with the `worktree-isolation-guard` hook. |
| `secrets-in-shell` | Fetch ad-hoc secrets at point of use (OS keychain / secrets manager); read only via `$(…)` command substitution — never print a secret to the transcript. Pairs with the `secret-leak-guard` hook. |
| `commit-cadence`  | Commit early and often (≈ every 15 min or after each fix), WIP commits included — favour more saved checkpoints over a clean-but-empty history. |
