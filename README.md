...
| `supply-chain-guard.py`        | `PreToolUse`  | Non-blocking reminder on package-install commands (`npm`/`pnpm`/`yarn`/`bun install\|add\|ci`, `pip`/`pipx install`, `uv add`/`uv pip install`, `poetry add`). Flags supply-chain risks and the safer invocation: git/URL sources (bypass registry review), missing `--ignore-scripts` (npm lifecycle hooks run arbitrary code), a bare `install` with a lockfile present (use `npm ci` / `--frozen-lockfile`), Python alternate indexes (`--index-url`/`--extra-index-url` dependency-confusion, `--trusted-host` TLS-off), and a new-package cooldown reminder. Ecosystem-agnostic checks apply to all; `--ignore-scripts`/lockfile-steering are npm-only; index/`--trusted-host` are pip/uv/poetry-only. Reinforces `research-before-build` + `security-audit`. Bypass: `CC_SUPPLY_CHAIN_OK=1`. |
