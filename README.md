...
| ship-pr                     | Workflow orchestrator (`/ship-pr`) — heavyweight pre-PR pipeline: the `/ship` gates **plus** `/code-review` (gate on Block) → conditional `security-review` → (confirm) `git-committer` → open a DRAFT PR (confirm). Review runs once here, not per commit. Optional tail: `review <pr#>` for an already-open PR. Model-invocable and ships a `claude_md.md` rule, so opening a PR routes through it automatically (best practice).                                                                                                                                                   |
