...
| Path | What it holds |
| --- | --- |
| `src/claude_all/cli.py` | The entire installer: resource discovery (`discover`), dependency-closure resolution (`resolve_closure`/`load_requires`), transparent Claude/Codex artifact installation (`install_item`), generated Codex TOML agents (`render_codex_agent`), `CLAUDE.md`/`AGENTS.md` snippet injection, Claude `settings.json` and Codex `hooks.json` wiring, an interactive curses TUI (`TuiState`, `tui_select`), and an `--uninstall` path that reverses recorded artifacts via a local state file. |
...
**Installing.** Running `claude-all` opens an interactive picker (or, non-interactively, `claude-all --all --user|--project <filters>`) and detects the locally available Claude and Codex CLIs. Every invocation content-fingerprints the Claude-authored agent sources and, when they changed, rebuilds generated Codex agent TOML under the single installer-owned cache at `~/.claude-all/codex/agents/`. Only selected agents are linked from that cache into Codex's standard locations, including project-local installs. Claude artifacts use `.claude`, while Codex receives linked `.codex/agents/*.toml`, direct `.agents/skills` links, `AGENTS.md` blocks, and `.codex/hooks.json` entries. The Claude Markdown agent remains the source of truth; its model alias is mapped to a Codex model only in the generated TOML. `--rebuild` performs only the agent-cache check and rebuild; it does not relink or otherwise change Codex-visible files. MCPs use their native CLI registration and tools are installed once because they are machine-wide. A full `--uninstall` also removes the managed cache after the last recorded install, without changing hand-authored configuration.
